...
const Contact = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      mobile: Yup.string().matches(/^[0-9]+$/, 'Must be a valid number').required('Mobile No. is required'),
  ...
  return (
    <div>
      ...
      <div className="container">
        <div className='contact-main-img'><img src={conactImage} alt="Contact" /></div>
          ...
          <div className="col-70 contact-form-top">
            ...
            <form onSubmit={formik.handleSubmit}>
              <div className='col-60 full-name-head'>
                ...
                <input type="text" placeholder='Enter Full Name' {...formik.getFieldProps('fullName')} />
              ...
              <div className='col-60 full-name-head'>
                ...
                <input type="text" placeholder='General' {...formik.getFieldProps('queryType')} />
                {formik.touched.queryType && formik.errors.queryType && <div className="error">{formik.errors.queryType}</div>}
              </div>
              <div className='col-60 full-name-head'>
                <div className='full-name'>Email</div>
                <input type="email" placeholder='Enter Email Here' {...formik.getFieldProps('email')} />
                {formik.touched.email && formik.errors.email && <div className="error">{formik.errors.email}</div>}
              </div>
              <div className='col-60 full-name-head'>
                <div className='full-name'>Mobile No.</div>
                <div className="flex align-center space-bw mobile-head">
                  <div className='col-20'><input type="text" placeholder='+971' {...formik.getFieldProps('mobilePrefix')} /></div>
                  <div className='col-80'><input type="text" placeholder='Enter Mobile No. Here' {...formik.getFieldProps('mobile')} /></div>
                </div>
                {formik.touched.mobile && formik.errors.mobile && <div className="error">{formik.errors.mobile}</div>}
              </div>
              <div className='col-60 full-name-head'>
                <div className='full-name'>Comment</div>
                <textarea placeholder='Enter Comments Here' {...formik.getFieldProps('comment')}></textarea>
                {formik.touched.comment && formik.errors.comment && <div className="error">{formik.errors.comment}</div>}
              </div>
              <button className='contact-submit col-20' type="submit">Submit</button>
            </form>
          </div>
        </div>
      </div>
